<!-- HEADER -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,18,20,24,30&height=220&section=header&text=Heo%20Sungjae&fontSize=55&fontAlignY=38&desc=Backend%20Developer&descAlignY=58&descSize=22&animation=fadeIn" width="100%"/>

<div align="center">

### 👋 Hi, I'm Sungjae

**Computer Engineering Student @ Hansung University**

Backend development을 중심으로  
**AI · Web · Web3** 기술을 활용해 실제 문제를 해결하는 서비스를 만드는 개발자입니다.

<br/>

[![GitHub](https://img.shields.io/badge/GitHub-SJ01--max-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/SJ01-max)
[![Email](https://img.shields.io/badge/Email-Contact-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:YOUR_EMAIL)

</div>

---

## 👨‍💻 About Me

- 🎓 **Hansung University** Computer Engineering
- 💻 Interested in **Backend Development**
- 🤖 Experience with **AI / RAG / Multimodal AI**
- 🌐 Experience building services with **React, FastAPI, Spring Boot**
- 🗄️ Worked with **PostgreSQL / pgvector**
- ☁️ Interested in **AWS / Docker / Cloud Infrastructure**
- ⛓️ Exploring **Web3 & Blockchain**
- 🤝 Experience as a **Team Leader** in multiple software projects

---

## 🛠 Tech Stack

### Backend

<p>
  <img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring%20Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white"/>
  <img src="https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white"/>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
</p>

### Frontend

<p>
  <img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black"/>
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white"/>
</p>

### Database

<p>
  <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white"/>
  <img src="https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white"/>
  <img src="https://img.shields.io/badge/pgvector-336791?style=for-the-badge&logo=postgresql&logoColor=white"/>
</p>

### DevOps & Tools

<p>
  <img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white"/>
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"/>
  <img src="https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</p>

---

# 🚀 Projects

## 🖥️ UIBowl
### Multimodal Mobile UI Pattern Classification

> Mobile app screenshots and flow sequences are analyzed to classify UI patterns automatically.

**Role**
- Team Leader
- React / TypeScript Frontend
- FastAPI API Integration
- CORS Configuration
- Vercel Deployment

**Tech**

`React` `TypeScript` `FastAPI` `EfficientNet-B0` `Attention LSTM` `Python` `Vercel`

**Key Features**
- Mobile UI screenshot analysis
- Top-3 UI pattern recommendation
- Flow timeline visualization
- Real-time screenshot upload and inference result visualization

---

## 🤖 PMI RAG System
### Natural Language Panel Extraction & Recommendation

> A system that extracts panels from natural-language queries and provides AI-based recommendations.

**Role**
- Team Leader
- Database Design & Management
- RAG System Development

**Tech**

`React` `Python` `LangChain` `PostgreSQL` `pgvector` `LLM`

**Key Features**
- Natural language query processing
- Vector similarity search
- RAG-based recommendation
- LLM Gateway architecture
- PostgreSQL vector database integration

---

## 📱 Trip충
### Chungcheong Travel Recommendation Service

> A travel service recommending seasonal destinations in the Chungcheong region.

**Role**
- Frontend Developer

**Tech**

`React` `TourAPI` `Weather API` `REST API`

**Key Features**
- Seasonal travel recommendations
- Tourism information API integration
- Weather information
- Transportation information

---

## 🧊 SmartFridge
### Smart Refrigerator Management Application

> A mobile application that manages refrigerator ingredients and recommends recipes.

**Tech**

`Flutter` `Spring Boot` `REST API`

**Key Features**
- User authentication
- Ingredient management
- Recipe recommendation
- Backend API integration

---

## 👥 UMC Project

> Collaborative web service development through UMC.

**Role**
- Frontend Team Leader
- Frontend Architecture
- Git Branch Strategy & Collaboration Management

**Tech**

`React` `TypeScript` `Git` `GitHub`

---

# 📚 Currently Learning

```text
Backend Architecture
Spring Boot
Docker
AWS
Database Optimization
System Design
Blockchain / Web3
